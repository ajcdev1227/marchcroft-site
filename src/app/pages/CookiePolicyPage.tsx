...
export const CookiePolicyPage = () => {
  const [cookiePolicyContent, setCookiePolicyContent] = useState<any>(null);
    ...
    const fetchCookiePolicy = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/api/privacies?filters[title]=cookie-policy`,
          {
            headers: {
        ...
        const cookiePolicyData = data.data[0]?.attributes?.json;

        if (cookiePolicyData) {
          setCookiePolicyContent(cookiePolicyData);
        } else {
          setError("No cookie policy content found.");
        }
      } catch (err) {
        console.error(error);
        setError("Failed to fetch cookie policy content.");
      } finally {
        setLoading(false);
      }
    };
