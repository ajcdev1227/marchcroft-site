...
import { CareersCallToActionSection } from "../../pages/Careers/CareersCallToActionSection";
import { AppDispatch, RootState } from "@/store";
import { selectTalkToUsSection, fetchContent, selectCareersPage } from "@/store/contentSlice";
...
const CareersPage = () => {
  ...
  const { sectionContent, status: contentStatus, error: contentError } = useSelector((state: RootState) => state.content);
  const talkToUs = useSelector(selectTalkToUsSection);
  const careersPage = useSelector(selectCareersPage);
    ...
    <section className='flex flex-col justify-center'>
      ...
      <section className='flex justify-center px-5 mt-10 bg-white z-10 py-10 lg:px-14 md:px-5 sm:px-5'>
        <section className='flex flex-col max-w-[1111px] container w-full mx-auto'>
          {/* CareerSection */}
          <CareersCallToActionSection content={careerData} title={careersPage.header}/>
        </section>
      </section>
